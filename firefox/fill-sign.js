...
function initFillSign() {
  ...
  // Attach event listeners to tool buttons (Text, Signature, Date, Strike, Redact, Delete)
  ...
  const strikeToolBtn = document.getElementById("strikeToolBtn");
  ...
  if (strikeToolBtn) {
    strikeToolBtn.addEventListener("click", () => activateTool("strike"));
    console.log("Strike tool button event listener attached");
  }
  if (deleteToolBtn) {
    deleteToolBtn.addEventListener("click", () => activateTool("delete"));
    console.log("Delete tool button event listener attached");
  }
...
// Activates a selected tool (Text, Signature, Date, Strike) and updates UI accordingly.
function activateTool(tool) {
  ...
  if (editorLayer) {
    // Remove tool visual indicators for all tools
    editorLayer.classList.remove("delete-tool-active");
    editorLayer.classList.remove("strike-tool-active");
    
    if (tool === "text" || tool === "signature" || tool === "eraser" || tool === "strike" || tool === "delete") {
      editorLayer.style.pointerEvents = "auto";
      console.log("Pointer events enabled for tool:", tool);
    } else {
      editorLayer.style.pointerEvents = "none";
      console.log("Pointer events disabled for tool:", tool);
    }
  }
  ...
  if (tool === "text") {
    ...
    if (textPropertiesPanel) textPropertiesPanel.classList.add("hidden");
    if (signaturePanel) signaturePanel.classList.add("hidden");
    if (editorLayer) editorLayer.classList.add("strike-tool-active");
    console.log("Strike tool activated, panels hidden");
  } else if (tool === "delete") {
    if (textPropertiesPanel) textPropertiesPanel.classList.add("hidden");
    if (signaturePanel) signaturePanel.classList.add("hidden");
    if (editorLayer) editorLayer.classList.add("delete-tool-active");
    console.log("Delete tool activated, panels hidden");
  }
}
...
function setupEditorEvents() {
  ...
  editorLayer.addEventListener("mousedown", (e) => {
    if (!activeTool || !pdfViewport) return;
    if (e.target.closest('.editor-element')) return;
    
    const rect = editorLayer.getBoundingClientRect();
    const x = e.clientX - rect.left;
    const y = e.clientY - rect.top;
    
    console.log("Mouse down event:", { activeTool, x, y });
    
    if (activeTool === "strike") {
      strikeStart = { x, y };
      console.log("Strike start set:", strikeStart);
      
      // Create preview element
      strikePreview = document.createElement("div");
      strikePreview.className = "strike-preview";
      strikePreview.style.left = `${x}px`;
      strikePreview.style.top = `${y}px`;
      strikePreview.style.width = "0px";
      strikePreview.style.height = "0px";
      
      // Use the selected color from the color picker
      const colorPicker = document.getElementById("strikeColorPicker");
      const color = colorPicker ? colorPicker.value : "#e11d48";
      strikePreview.style.background = color;
      
      editorLayer.appendChild(strikePreview);
    }
  });

  editorLayer.addEventListener("mousemove", (e) => {
    if (!activeTool || !pdfViewport) return;
    if (e.target.closest('.editor-element')) return;
    ...
    if (activeTool === "strike" && strikeStart && strikePreview) {
      const width = Math.abs(x - strikeStart.x);
      const height = Math.abs(y - strikeStart.y);
      const left = Math.min(x, strikeStart.x);
      const top = Math.min(y, strikeStart.y);
      
      strikePreview.style.left = `${left}px`;
      strikePreview.style.top = `${top}px`;
      strikePreview.style.width = `${width}px`;
      strikePreview.style.height = `${height}px`;
    }
  });

  editorLayer.addEventListener("mouseup", (e) => {
    if (!activeTool || !pdfViewport) return;
    if (e.target.closest('.editor-element')) return;
    ...
    console.log("Mouse up event:", { activeTool, x, y, strikeStart });
    
    if (activeTool === "strike" && strikeStart) {
      console.log("Creating strike element from", strikeStart, "to", { x, y });
      addStrikeElement(strikeStart.x, strikeStart.y, x, y);
      strikeStart = null;
      
      // Remove preview element
      if (strikePreview) {
        strikePreview.remove();
        strikePreview = null;
      }
    }
  });
  ...
  editorLayer.addEventListener("click", (e) => {
    console.log("Click event:", { activeTool, target: e.target.className });
    ...
    // Handle delete tool clicks on elements
    if (activeTool === "delete" && e.target.closest('.editor-element')) {
      const clickedElement = e.target.closest('.editor-element');
      console.log("Delete tool: clicked on element:", clickedElement.dataset.id, "Type:", clickedElement.dataset.type);
      deleteElement(clickedElement);
      return;
    }
    
    // For other tools, don't add new elements if clicking on existing ones
...
function addTextElement(x, y) {
  ...
  textElement.style.color = (document.getElementById("fontColor").value && document.getElementById("fontColor").value.startsWith('#')) ? document.getElementById("fontColor").value : "#000000";   
...
function makeElementDraggable(element) {
  ...
  element.addEventListener("mousedown", (e) => {
    // Disable dragging when delete tool is active
    if (activeTool === "delete") {
      console.log("Delete tool active, preventing drag on element:", element.dataset.id);
      return;
    }
...
function selectElement(element) {
  ...
  if (element.dataset.type === "text" || element.dataset.type === "date") {
    if (textPropertiesPanel) {
      ...
      const fontColor = (element.style.color && element.style.color.startsWith('#')) ? element.style.color : "#000000";
...
function applyTextProperties() {
  ...
  const fontColor = (document.getElementById("fontColor")?.value && document.getElementById("fontColor").value.startsWith('#')) ? document.getElementById("fontColor").value : "#000000";
...
function storeEditorElement(element) {
  ...
  const elementData = {
    ...
    color: (element.style.color && element.style.color.startsWith('#')) ? element.style.color : "#000000",
    ...
    // For strike
    x1: element.dataset.x1,
    y1: element.dataset.y1,
    x2: element.dataset.x2,
    y2: element.dataset.y2,
    strikeColor: element.dataset.color && element.dataset.color.startsWith('#') ? element.dataset.color : '#e11d48',
  };
...
function updateStoredElement(element) {
  ...
  if (existingElementData) { 
    ...
    if (element.dataset.type === 'text' || element.dataset.type === 'date') {
      ...
      existingElementData.color = (element.style.color && element.style.color.startsWith('#')) ? element.style.color : "#000000";
...
function restoreEditorElements() {
  ...
  pageElements.forEach(elData => {
    ...
    if (elData.type === "text" || elData.type === "date") {
      ...
      element.style.color = (elData.color && elData.color.startsWith('#')) ? elData.color : "#000000";
    ...
    } else if (elData.type === "strike") {
      const strikeDiv = document.createElement("div");
      strikeDiv.className = "editor-element strike-element";
      strikeDiv.style.left = `${elData.x1}px`;
      strikeDiv.style.top = `${elData.y1}px`;
      strikeDiv.style.width = `${elData.x2 - elData.x1}px`;
      strikeDiv.style.height = `${elData.y2 - elData.y1}px`;
      strikeDiv.style.background = (elData.strikeColor && elData.strikeColor.startsWith('#')) ? elData.strikeColor : "#e11d48";
      strikeDiv.dataset.type = "strike";
      strikeDiv.dataset.page = elData.page.toString();
      strikeDiv.dataset.x1 = elData.x1;
      strikeDiv.dataset.y1 = elData.y1;
      strikeDiv.dataset.x2 = elData.x2;
      strikeDiv.dataset.y2 = elData.y2;
      strikeDiv.dataset.color = (elData.strikeColor && elData.strikeColor.startsWith('#')) ? elData.strikeColor : "#e11d48";
      editorLayer.appendChild(strikeDiv);
      makeElementDraggable(strikeDiv);
    } else if (elData.type === "redact") {
      const redactDiv = document.createElement("div");
      redactDiv.className = "editor-element redact-element";
      redactDiv.style.left = `${elData.x1}px`;
      redactDiv.style.top = `${elData.y1}px`;
      redactDiv.style.width = `${elData.x2 - elData.x1}px`;
      redactDiv.style.height = `${elData.y2 - elData.y1}px`;
      redactDiv.style.background = "#000000";
      redactDiv.dataset.type = "redact";
      redactDiv.dataset.page = elData.page.toString();
      redactDiv.dataset.x1 = Math.min(elData.x1, elData.x2);
      redactDiv.dataset.y1 = Math.min(elData.y1, elData.y2);
      redactDiv.dataset.x2 = Math.max(elData.x1, elData.x2);
      redactDiv.dataset.y2 = Math.max(elData.y1, elData.y2);
      editorLayer.appendChild(redactDiv);
      makeElementDraggable(redactDiv);
    }
...
function eraseElementAtPosition(x, y) {
  ...
  if (elementToErase) {
    ...
    const elementData = {
      ...
      color: (elementToErase.style.color && elementToErase.style.color.startsWith('#')) ? elementToErase.style.color : "#000000",
...
// Deletes a specific element (for delete tool)
function deleteElement(elementToDelete) {
  if (!elementToDelete) {
    console.error("No element provided to deleteElement");
    return;
  }

  console.log("deleteElement called for:", elementToDelete.dataset.id, "Type:", elementToDelete.dataset.type);

  // Store the element data for undo
  const elementData = {
    id: elementToDelete.dataset.id,
    type: elementToDelete.dataset.type,
    page: parseInt(elementToDelete.dataset.page),
    left: elementToDelete.style.left,
    top: elementToDelete.style.top,
    width: elementToDelete.style.width,
    height: elementToDelete.style.height,
    fontSize: elementToDelete.style.fontSize,
    color: (elementToDelete.style.color && elementToDelete.style.color.startsWith('#')) ? elementToDelete.style.color : "#000000",
    content: elementToDelete.textContent || '',
    src: elementToDelete.dataset.src || ''
  };

  // Add strike-specific data if it's a strike element
  if (elementToDelete.dataset.type === "strike") {
    elementData.x1 = elementToDelete.dataset.x1;
    elementData.y1 = elementToDelete.dataset.y1;
    elementData.x2 = elementToDelete.dataset.x2;
    elementData.y2 = elementToDelete.dataset.y2;
    elementData.strikeColor = elementToDelete.dataset.color;
    elementData.thickness = elementToDelete.dataset.thickness;
  }

  // Add redact-specific data if it's a redact element
  if (elementToDelete.dataset.type === "redact") {
    elementData.x1 = elementToDelete.dataset.x1;
    elementData.y1 = elementToDelete.dataset.y1;
    elementData.x2 = elementToDelete.dataset.x2;
    elementData.y2 = elementToDelete.dataset.y2;
  ...
  console.log("Element deleted:", elementId, "Type:", elementData.type);
}
...
function undoLastAction() {
  ...
  if (lastAction.type === 'erase') {
    ...
    restoreDeletedElement(lastAction.element);
  }
...
function restoreErasedElement(elementData) {
  ...
  if (elementData.type === "text" || elementData.type === "date") {
    ...
    element.style.maxWidth = "200px";
    element.style.maxHeight = "100px";
  } else if (elementData.type === "strike") {
    const strikeDiv = document.createElement("div");
    strikeDiv.className = "editor-element strike-element";
    strikeDiv.style.left = `${elementData.x1}px`;
    strikeDiv.style.top = `${elementData.y1}px`;
    strikeDiv.style.width = `${elementData.x2 - elementData.x1}px`;
    strikeDiv.style.height = `${elementData.y2 - elementData.y1}px`;
    strikeDiv.style.background = (elementData.strikeColor && elementData.strikeColor.startsWith('#')) ? elementData.strikeColor : "#e11d48";
    strikeDiv.dataset.type = "strike";
    strikeDiv.dataset.page = elementData.page.toString();
    strikeDiv.dataset.x1 = elementData.x1;
    strikeDiv.dataset.y1 = elementData.y1;
    strikeDiv.dataset.x2 = elementData.x2;
    strikeDiv.dataset.y2 = elementData.y2;
    strikeDiv.dataset.color = (elementData.strikeColor && elementData.strikeColor.startsWith('#')) ? elementData.strikeColor : "#e11d48";
    editorLayer.appendChild(strikeDiv);
    makeElementDraggable(strikeDiv);
  } else if (elementData.type === "redact") {
    const redactDiv = document.createElement("div");
    redactDiv.className = "editor-element redact-element";
    redactDiv.style.left = `${elementData.x1}px`;
    redactDiv.style.top = `${elementData.y1}px`;
    redactDiv.style.width = `${elementData.x2 - elementData.x1}px`;
    redactDiv.style.height = `${elementData.y2 - elementData.y1}px`;
    redactDiv.style.background = "#000000";
    redactDiv.dataset.type = "redact";
    redactDiv.dataset.page = elementData.page.toString();
    redactDiv.dataset.x1 = Math.min(elementData.x1, elementData.x2);
    redactDiv.dataset.y1 = Math.min(elementData.y1, elementData.y2);
    redactDiv.dataset.x2 = Math.max(elementData.x1, elementData.x2);
    redactDiv.dataset.y2 = Math.max(elementData.y1, elementData.y2);
    editorLayer.appendChild(redactDiv);
    makeElementDraggable(redactDiv);
  }
  ...
  if (element) {
    ...
    element.style.color = (elementData.color && elementData.color.startsWith('#')) ? elementData.color : "#000000";
  ...
  console.log("Deleted element restored:", elementData.id, "Type:", elementData.type);
}

// Restores a deleted element
function restoreDeletedElement(elementData) {
  const editorLayer = document.getElementById("editorLayer");
  if (!editorLayer) return;
    ...
    for (let i = 0; i < pdfDoc.getPageCount(); i++) {
      ...
      for (const el of pageElementsToEmbed) {
        ...
        if (el.type === "text" || el.type === "date") {
          ...
          const colorComps = parseColorToRGBComponents(el.color || "#000000");
        ...
        } else if (el.type === "strike") {
          const x1_pdf = parseFloat(el.x1) / scaleForSaving;
          const y1_pdf = pdfPageHeight - (parseFloat(el.y1) / scaleForSaving);
          const x2_pdf = parseFloat(el.x2) / scaleForSaving;
          const y2_pdf = pdfPageHeight - (parseFloat(el.y2) / scaleForSaving);
          const width = x2_pdf - x1_pdf;
          const height = y1_pdf - y2_pdf;
          const colorComps = parseColorToRGBComponents(el.strikeColor || "#e11d48");
          console.log(`Drawing strikethrough rectangle at (x:${x1_pdf}, y:${y2_pdf}), w:${width}, h:${height}`);
          page.drawRectangle({
            x: x1_pdf,
            y: y2_pdf,
            width: width,
...
// Adds a strike element to the editor layer
function addStrikeElement(x1, y1, x2, y2) {
  console.log("addStrikeElement called with:", { x1, y1, x2, y2 });
  
  const editorLayer = document.getElementById("editorLayer");
  if (!editorLayer) {
    console.error("Editor layer not found in addStrikeElement");
    return;
  }
  ...
  // Check for minimum size (at least 10x10 pixels)
  if (width < 10 || height < 10) {
    console.log("Strikethrough rectangle too small, ignoring:", { width, height });
    return;
  }
  ...
  console.log("Strikethrough rectangle dimensions:", { left, top, width, height, color });
  
  const strikeDiv = document.createElement("div");
  strikeDiv.className = "editor-element strike-element";
  strikeDiv.style.left = `${left}px`;
  strikeDiv.style.top = `${top}px`;
  strikeDiv.style.width = `${width}px`;
  strikeDiv.style.height = `${height}px`;
  strikeDiv.style.background = color;
  strikeDiv.dataset.type = "strike";
  strikeDiv.dataset.page = currentPageIndex.toString();
  strikeDiv.dataset.x1 = left;
  strikeDiv.dataset.y1 = top;
  strikeDiv.dataset.x2 = left + width;
  strikeDiv.dataset.y2 = top + height;
  strikeDiv.dataset.color = color;
  
  console.log("Strike element created:", {
    left: strikeDiv.style.left,
    top: strikeDiv.style.top,
    width: strikeDiv.style.width,
    height: strikeDiv.style.height,
    background: strikeDiv.style.background,
    dataset: {
      x1: strikeDiv.dataset.x1,
      y1: strikeDiv.dataset.y1,
      x2: strikeDiv.dataset.x2,
      y2: strikeDiv.dataset.y2,
      color: strikeDiv.dataset.color
    }
  });
  
  editorLayer.appendChild(strikeDiv);
  makeElementDraggable(strikeDiv);
  storeEditorElement(strikeDiv);
  addToUndoStack({ type: 'add', elementId: strikeDiv.dataset.id });
  
  console.log("Strike element added to editor layer");
}

// Adds a redaction element to the editor layer
function addRedactElement(x1, y1, x2, y2) {
  console.log("addRedactElement called with:", { x1, y1, x2, y2 });
  
  const editorLayer = document.getElementById("editorLayer");
  if (!editorLayer) {
    console.error("Editor layer not found in addRedactElement");
    return;
  }
  ...
  // Check for minimum size (at least 10x10 pixels)
  if (width < 10 || height < 10) {
    console.log("Redaction rectangle too small, ignoring:", { width, height });
    return;
  }
  ...
  console.log("Redaction rectangle dimensions:", { left, top, width, height });
  
  const redactDiv = document.createElement("div");
  redactDiv.className = "editor-element redact-element";
  redactDiv.style.left = `${left}px`;
  redactDiv.style.top = `${top}px`;
  redactDiv.style.width = `${width}px`;
  redactDiv.style.height = `${height}px`;
  redactDiv.style.background = "#000000";
  redactDiv.dataset.type = "redact";
  redactDiv.dataset.page = currentPageIndex.toString();
  redactDiv.dataset.x1 = left;
  redactDiv.dataset.y1 = top;
  redactDiv.dataset.x2 = left + width;
  redactDiv.dataset.y2 = top + height;
  
  console.log("Redact element created:", {
    left: redactDiv.style.left,
    top: redactDiv.style.top,
    width: redactDiv.style.width,
    height: redactDiv.style.height,
    dataset: {
      x1: redactDiv.dataset.x1,
      y1: redactDiv.dataset.y1,
      x2: redactDiv.dataset.x2,
      y2: redactDiv.dataset.y2
    }
  });
  
  editorLayer.appendChild(redactDiv);
  makeElementDraggable(redactDiv);
  storeEditorElement(redactDiv);
  addToUndoStack({ type: 'add', elementId: redactDiv.dataset.id });
  
  console.log("Redact element added to editor layer");
}
